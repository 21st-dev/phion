...
export const detectOS = (): "mac" | "windows" | "linux" => {
  if (typeof window === "undefined") return "mac" // SSR fallback
...
// Generate CLI commands for different platforms
export const generateCliCommand = (projectId: string, os: "mac" | "windows" | "linux"): string => {
  ...
  switch (os) {
    ...
    case "windows":
      return `Invoke-WebRequest -Uri "${apiUrl}" -OutFile "project.zip" -UseBasicParsing; Expand-Archive -Path "project.zip" -DestinationPath "." -Force; Remove-Item "project.zip"; $folder = Get-ChildItem -Directory | Sort-Object LastWriteTime -Descending | Select-Object -First 1; Move-Item "$($folder.Name)\\*" -Destination "." -Force; Get-ChildItem "$($folder.Name)" -Hidden | Move-Item -Destination "." -Force; Remove-Item $folder.Name -Recurse -Force; icacls setup.ps1 /grant Everyone:RX; .\\setup.ps1`
...
export interface CLIProjectInstallProps {
  projectId: string
  className?: string
}
            ...
            <CodeBlock className="rounded-md">
              <div className="flex items-center justify-between p-3 border-b border-border bg-muted/50">
                ...
                <Button
                  ...
                  onClick={() =>
                    handleCopyCommand(
                      generateCliCommand(projectId, key as "mac" | "windows" | "linux"),
                    )
                  }
                  className="h-7 px-2"
                >
              ...
              <CodeBlockCode
                code={generateCliCommand(projectId, key as "mac" | "windows" | "linux")}
