...
export function useWebSocket({ 
  ...
  useEffect(() => {
    ...
    if (!projectId || isConnecting.current) {
      console.log(`⏭️ [WebSocket] Skipping connection - projectId: ${!!projectId}, isConnecting: ${isConnecting.current}`);
      return;
    }
    ...
    console.log(`🔌 [WebSocket] Initializing connection for project: ${projectId} (ID: ${connectionId.current})`);
