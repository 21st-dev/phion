...
export function useWebSocket({
  ...
  useEffect(() => {
    if (projectId && projectId !== lastProjectId.current) {
      ...
      console.log(
        `🔑 [WebSocket] Generated connection ID: ${connectionId.current}`,
      );
    }
  }, [projectId]);
  ...
  useEffect(() => {
    ...
    if (socket && socket.connected) {
      console.log(
        `⏭️ [WebSocket] Already connected for project ${projectId}, skipping`,
      );
      return;
    }
    ...
    connectionTimeout.current = setTimeout(() => {
      ...
      console.log(
        `🚀 [WebSocket] Creating socket connection (ID: ${connectionId.current})`,
      );
      ...
      newSocket.on("disconnect", (reason) => {
        ...
        if (
          !clientInitiated.includes(reason) &&
