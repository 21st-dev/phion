...
export async function GET(
  ...
  try {
    ...
    console.log(
      `🔄 [DOWNLOAD] Starting GitHub-based download for project ${projectId}`,
    );
    ...
    console.log(
      `✅ [DOWNLOAD] Project found: ${project.name} (template: ${project.template_type})`,
    );

    // Проверяем, что у проекта есть GitHub репозиторий
    if (!project.github_repo_name) {
      console.log(
        `❌ [DOWNLOAD] Project ${projectId} has no GitHub repository`,
      );
    ...
    console.log(
      `⬇️ [DOWNLOAD] Downloading ZIP from GitHub repository: ${project.github_repo_name}`,
    );
    ...
    try {
      ...
      console.error(
        `❌ [DOWNLOAD] GitHub download failed for project ${projectId}:`,
        downloadError,
      );
      return NextResponse.json(
    ...
    if (!originalProjectData || originalProjectData.length === 0) {
      console.log(
        `❌ [DOWNLOAD] Empty or invalid project data from GitHub for ${projectId}`,
      );
      return NextResponse.json(
    ...
    console.log(
      `🔄 [DOWNLOAD] Processing ZIP to rename root folder for project ${projectId}...`,
    );
    ...
    try {
      ...
      console.log(
        `📂 [DOWNLOAD] Original ZIP contains ${entries.length} entries`,
      );
      ...
      if (firstEntry) {
        ...
        console.log(
          `📁 [DOWNLOAD] Original root folder: "${originalRootFolder}"`,
        );
      }
      ...
      entries.forEach((entry) => {
        ...
        if (
          originalRootFolder &&
          entry.entryName.startsWith(originalRootFolder)
        ) {
        ...
        if (
          ...
          console.log(
            `🔄 [DOWNLOAD] Processed ${processedEntries}/${entries.length} entries`,
          );
        }
      });
      ...
      console.log(
        `⚠️ [DOWNLOAD] Falling back to original ZIP for project ${projectId}`,
      );
    ...
    console.log(
      `🎉 [DOWNLOAD] Successfully completed download for ${projectId} in ${totalTime}ms`,
    );
    ...
    console.log(
      `📁 [DOWNLOAD] Filename: "${originalFileName}" → "${fullFileName}"`,
    );
