...
console.log("🔧 Phion Diagnostic Tool")
console.log("========================\n")
...
try {
  ...
  console.log("⚠️  Could not read phion.config.json")
}
...
async function checkEnvironment() {
  ...
  if (DEBUG_MODE) {
    ...
    Object.keys(process.env)
      .filter((key) => key.toLowerCase().includes("cursor") || key.toLowerCase().includes("vscode"))
      .forEach((key) => {
  ...
  console.log(`   Cursor command available: ${cursorCommandAvailable ? "✅" : "❌"}`)
...
async function checkExtension() {
  ...
  console.log(`   Extension installed in Cursor: ${extensionInstalled ? "✅" : "❌"}`)
...
async function checkDevServer() {
  ...
  console.log(`   WebSocket server (localhost:8080): ${wsServerRunning ? "✅" : "❌"}`)
...
async function provideSolutions(checks) {
  ...
  if (!checks.environment.cursorDetected) {
    console.log("\n❌ Cursor not detected:")
    console.log("   • Make sure you're running this from within Cursor terminal")
...
async function main() {
  try {
    ...
    console.log("\n🔧 Diagnostic complete!")
    console.log("If problems persist, check: https://docs.phion.com/troubleshooting")
  } catch (error) {
