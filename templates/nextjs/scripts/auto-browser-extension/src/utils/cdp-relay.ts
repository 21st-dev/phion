...
export class CDPRelayServer extends EventEmitter {
  ...
  private _connectionInfo:
    ...
    | undefined
  private _consoleLogCallback: ((log: any) => void) | null = null
  ...
  setConsoleLogCallback(callback: (log: any) => void): void {
    this._consoleLogCallback = callback
  }
  ...
  private _handlePlaywrightConnection(ws: WebSocket): void {
    ...
    ws.on("close", () => {
      if (this._playwrightSocket === ws) this._playwrightSocket = null
  ...
  private _handleExtensionConnection(ws: WebSocket): void {
    ...
    ws.on("close", () => {
      if (this._extensionSocket === ws) this._extensionSocket = null
    ...
    // Auto-enable Runtime domain for console logs when extension connects
    setTimeout(() => {
      this._enableRuntimeDomain()
    }, 1000)
  }
   ...
   */
  private _enableRuntimeDomain(): void {
    if (this._extensionSocket?.readyState === WebSocket.OPEN) {
      ...
      debugLogger("Auto-enabling Runtime domain for console logs")
      this._extensionSocket.send(JSON.stringify(enableRuntimeMessage))
    }
  }
  ...
  private _handlePlaywrightMessage(message: any): void {
    ...
    if (message.method) this._forwardToExtension(message)
  }
  ...
  private _handleExtensionMessage(message: any): void {
    ...
    debugLogger(
      `← Extension message: ${message.method ?? (message.id && `response(id=${message.id})`) ?? "unknown"}`,
    )

    // Handle console log events
    if (message.method === "Runtime.consoleAPICalled" && this._consoleLogCallback) {
      this._consoleLogCallback(message.params)
    }
...
export async function startCDPRelayServer(httpServer: http.Server) {
  ...
  console.error(
    `CDP relay server started on ${wsAddress}${EXTENSION_PATH} - Connect to it using the browser extension.`,
  )
  const cdpEndpoint = `${wsAddress}${CDP_PATH}`
  return { cdpEndpoint, server: cdpRelayServer }
}
