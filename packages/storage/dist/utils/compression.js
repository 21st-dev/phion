 ...
 */
export async function createProjectZip(files, // filePath -> content
projectName = "project") {
 ...
 */
export async function extractProjectZip(zipBuffer) {
    try {
 ...
 */
export async function createDirectoryZip(directoryPath, excludePatterns = ["node_modules", ".git", "dist", ".next"]) {
    const files = {};
 ...
 */
export async function extractZipToDirectory(zipBuffer, targetDirectory) {
 ...
 */
export function getArchiveSize(zipBuffer) {
    return zipBuffer.length;
}
/**
 * Проверить валидность ZIP архива
 */
export async function validateZipArchive(zipBuffer) {
    try {
 ...
 */
export async function listZipContents(zipBuffer) {
    try {
        const files = await extractProjectZip(zipBuffer);
 ...
 */
export function decompressText(buffer) {
    // TODO: Реализовать с помощью zlib
    // Пока что просто возвращаем как есть
...
export async function createArchiveManifest(files, name = "project", version = "1.0.0") {
