 ...
 */
export async function getProjectVersions(projectId: string): Promise<FileVersion[]> {
 ...
 */
export async function copyProjectVersion(
  projectId: string,
 ...
 */
export async function getProjectStructure(projectId: string): Promise<ProjectFileStructure> {
  try {
    const key = `projects/${projectId}/structure.json`;
    return await downloadJsonFile<ProjectFileStructure>(key);
  } catch (error) {
...
async function uploadFile(key: string, content: string): Promise<void> {
