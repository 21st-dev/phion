...
class TabShareExtension {
  ...
  async connectTab(tabId, bridgeUrl) {
    try {
      ...
      chrome.action.setTitle({ tabId, title: "Disconnect" })
  ...
  setupMessageHandling(connection) {
    ...
    socket.onmessage = async (event) => {
      ...
      try {
        ...
        if (sessionId && sessionId !== rootSessionId) debuggerSession.sessionId = sessionId
  ...
  async disconnectTab(tabId) {
    ...
    // Update UI
    chrome.action.setBadgeText({ tabId, text: "" })
    chrome.action.setTitle({ tabId, title: "Share tab" })
